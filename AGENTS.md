...
| **Agent harness (canonical)**    | `.apm/` + `apm.yml` + `apm.lock.yaml`                            |
| Agent harness (per-client deploy)| `.claude/`, `.cursor/`, `.codex/`, `.opencode/`, `.github/` — REGENERATED by `apm install`; never edit by hand |
...
The agent harness is shipped once under `.apm/` and deployed by `apm install` to every locally-installed AI coding tool's native config root. Supported clients today: Claude Code (`.claude/`), Codex CLI (`.codex/`, `.agents/`), Cursor (`.cursor/`), GitHub Copilot CLI (`.github/`), OpenCode (`.opencode/`). APM handles per-target translation (Codex agents become TOML, Cursor rules become `.mdc`, Claude hooks merge into `settings.json`).

You — the agent — never edit the per-client roots. They're regenerated from `.apm/` on every `apm install`. If you want to add a new slash command or skill, edit `.apm/prompts/<n>.prompt.md` or `.apm/skills/<n>/SKILL.md` and the human runs `apm install` to redeploy. (And note: editing `.apm/` is still outside the two writable surfaces — surface a change request to the human, who'll handle it.)

Three scripts may execute around your edits:

- `scripts/agent-surface-fence.mjs` — rejects writes outside the two allowed surfaces when its PreToolUse hook is wired in. Tool-agnostic: adapters for Claude/Codex/Cursor/Copilot tool-name vocabularies.
- `scripts/pipeline.mjs` — runs after every assistant turn (Stop hook in Claude / Codex; manual via `pnpm pipeline` otherwise). Lints, exports, rebuilds, snapshots, mirrors.
