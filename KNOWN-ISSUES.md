...
- `.dsx/references/` is bundled and pinned. The four upstream-borrowed files (`brutalist.md`, `consumer-vivid.md`, `editorial-heritage.md`, `saas-minimal.md`) are derived from [VoltAgent/awesome-design-md](https://github.com/VoltAgent/awesome-design-md) and re-attributed in their description fields. They live at `/design-md/<brand>/DESIGN.md` upstream (not `/designs/` — the path moved at some point; document for future re-fetches).
- The DESIGN.md spec (`@google/design.md@0.1.1`) is silent on custom front-matter keys. Tested locally: a `keywords:` array passes lint with no error or warning. If a future spec version tightens, move keywords into an HTML comment block at the top of each reference and update `scripts/find-reference.mjs` to read from there.
- `brutalist.md` and `saas-minimal.md` carry SpaceX / Stripe lint warnings (unreferenced tokens) inherited from upstream. They're informational and intentionally preserved — references model real-world systems, not the dsx canonical shape. The active `DESIGN.md` (rendered by the gallery) is the source of truth that must lint clean.

## Pipeline + watcher

- `@google/design.md export --format tailwind` (v0.1.1) emits a v3-style JSON config (`theme.extend.colors`, ...), not a Tailwind v4 CSS file. The pipeline therefore reads DESIGN.md's front-matter directly (via `scripts/design-md-to-css.mjs`) and emits `apps/gallery/app/tokens.active.css` with both `:root` and `@theme inline` blocks. If the CLI grows a v4 CSS export, the pipeline can swap to it and drop the custom transform.
- `chokidar@5` is path-aware; on Windows, watching `DESIGN.md` (no slash) works in WSL but native Windows shells may want `./DESIGN.md`. Document if/when it actually breaks; current local dev is WSL.
- The Stop hook runs the full pipeline on every assistant turn. Hot pipeline = ~1.5 s on this machine; cold is ~3 s (npx fetch). If turn-over latency starts to matter, cache `@google/design.md` and run `node style-dictionary.config.mjs` directly instead of via `pnpm`.
