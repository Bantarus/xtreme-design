...
function persistVersion(id, name, brief, source) {
  ...
  const meta = {
    ...
    gitSha: gitSha(),
    brief,
    paletteHex: paletteOf(join(dir, 'tokens.css')),
  };
...
function ensureBase(idx) {
  ...
  const meta = {
    ...
    paletteHex: paletteOf(join(baseDir, 'tokens.css')),
  };
...
function autoMode(brief) {
  ...
  const activeDesignContent = readUtf8(ACTIVE_DESIGN);
  // Idempotency: if the last non-base version's DESIGN.md matches the active
  // one byte-for-byte, do not create a duplicate.
  const lastNonBase = [...idx].reverse().find((v) => v.id !== 'base');
  if (lastNonBase) {
    const prevDesign = readUtf8(join(VERSIONS_ROOT, lastNonBase.id, 'DESIGN.md'));
    if (prevDesign && prevDesign === activeDesignContent) {
      saveIndex(idx);
      process.stdout.write(`snapshot: no change since ${lastNonBase.id} — skipped.\n`);
      return;
    }
  }
